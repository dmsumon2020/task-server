...
/* app.post("/tasks", async (req, res) => {
  try {
    ...
    if (!title || !userId || !category) {
      return res
        .status(400)
        .json({ message: "Title, Category, and User ID are required" });
    }
    const newTask = {
      title,
      description,
...
app.post("/tasks", async (req, res) => {
  try {
    const { title, description, status, userId } = req.body; // Change "category" to "status"
    ...
    const newTask = {
      title,
      description,
      status, // Store status instead of category
      userId,
      createdAt: new Date(),
    };
...
/* app.get("/tasks/category/:category", async (req, res) => {
  try {
    ...
    const tasks = await tasksCollection.find({ userId, status }).toArray();
    res.status(200).json(tasks);
  } catch (error) {
    res.status(500).json({ message: "Error retrieving tasks", error });
  }
...
app.get("/tasks/category/:category", async (req, res) => {
  try {
    const { userId } = req.query;
    const { category } = req.params; // 'category' is actually the status
    if (!userId) {
      return res.status(400).json({ message: "User ID is required" });
    }
    // Fix the query to use status: category
...
/* app.put("/tasks/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body; // Update the status field
